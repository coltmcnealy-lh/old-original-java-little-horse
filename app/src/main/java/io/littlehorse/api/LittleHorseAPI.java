...
public class LittleHorseAPI {
    ...
    public LittleHorseAPI(DepInjContext config, Topology topology) {
        ...
        List<Class<? extends GETable>> resources =
                Arrays.asList(WFSpec.class, TaskDef.class, ExternalEventDef.class, WFRun.class);
        ...
        this.app.get("/internal/storeBytes/{storeName}/{storeKey}", this::getBytesFromStore);
    }

    private <T extends POSTable> void addPOSTApi(Class<T> cls) {
        postApis.add(new POSTApi<T>(this.config, cls, new APIStreamsContext<>(streams, cls, config),
                app));
    }

    private <T extends GETable> void addGETApi(Class<T> cls) {
        getApis.add(new GETApi<T>(this.config, cls, new APIStreamsContext<>(streams, cls, config),
                app));
    }
    ...
    private void getBytesFromStore(Context ctx) {
        ...
        ReadOnlyKeyValueStore<String, Bytes> store = streams.store(StoreQueryParameters
        ...
        rawResponse.status = result == null ? ResponseStatus.OBJECT_NOT_FOUND : ResponseStatus.OK;
        ctx.json(rawResponse);
    }

    /**
     * Idempotent LittleHorse cluster setup is currently done on startup of the LH Core API. All it
     * does right now is create a bunch of kafka topics so everybody is happy.
     */
    public static void doIdempotentSetup(DepInjContext config) {
        ...
        for (Class<? extends GETable> cls : Arrays.asList(WFSpec.class, TaskDef.class,
                ExternalEventDef.class, WFRun.class)) {
            LHUtil.log("About to create topics for ", cls.getName());
            config.createKafkaTopic(new NewTopic(POSTable.getIdKafkaTopic(config, cls), partitions,
                    replicationFactor));

            config.createKafkaTopic(new NewTopic(POSTable.getIndexKafkaTopic(config, cls),
